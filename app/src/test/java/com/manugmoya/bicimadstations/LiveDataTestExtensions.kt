 ...
 */
class LiveDataValueCapture<T: Any> {
    ...
    suspend fun assertSendsValues(timeout: Long, vararg expected: T) {
        ...
        try {
            ...
            Assert.assertTrue(values.containsAll(expectedList))
        }
    }

    suspend fun assertSendsAllValues(timeout: Long, vararg expected: T) {
        val expectedList = expected.asList()
        if (values == expectedList) {
            return
        }
    ...
    suspend fun assertSendsTypes(timeout: Long, vararg expected: T) {
        val expectedList = expected.asList()
        if (values::class == expectedList::class) {
            return
        }
        try {
            withTimeout(timeout) {
    ...
    suspend fun assertSendsAllTypes(timeout: Long, vararg expected: T) {
        val expectedList = expected.asList()
        if (values::class == expectedList::class) {
            return
        }
 ...
 */
inline fun <T: Any> LiveData<T>.captureValues(block: LiveDataValueCapture<T>.() -> Unit) {
